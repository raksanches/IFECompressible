...
//Create base points
	...
	Point* p2 = fluid->addPoint({ .35, 0. }, 0.01);
	Point* p3 = fluid->addPoint({ .35, .7 }, 0.01);
	Point* p4 = fluid->addPoint({ 0., .7 }, 0.01);
...
//setting number of line division for mesh creation: line, ndiv
	fluid->transfiniteLine({ l1 }, 35);
	fluid->transfiniteLine({ l2 }, 70); 
	fluid->transfiniteLine({ l3 }, 35);
	fluid->transfiniteLine({ l4 }, 70);
	

//adding boundary conditions
//  	fluid->addBoundaryCondition("NEUMANN", l1, {0.0}, {0.0}, "GLOBAL");
	fluid->addBoundaryCondition("DIRICHLET", l1, true, true, 0., 0.0, "GLOBAL");
	fluid->addBoundaryCondition("DIRICHLET", l2, false, false, 0.0, 0., "GLOBAL");
	fluid->addBoundaryCondition("DIRICHLET", l3, false, false, 0.0, 0., "GLOBAL");
	...
	fluid->addBoundaryCondition("MESHDISPLACEMENT", l2, false, false, 0.0, 0., "GLOBAL");
	fluid->addBoundaryCondition("MESHDISPLACEMENT", l3, false, false, 0., 0.0, "GLOBAL");
	fluid->addBoundaryCondition("MESHDISPLACEMENT", l4, true, false, 0.0, 0., "GLOBAL");
